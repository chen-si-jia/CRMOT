...
class Visualization(object):
    ...
    """
    def __init__(self, seq_info, exp_name, update_ms): 
        self.view_ls = list(seq_info.keys())
        key0 = self.view_ls[0]
        image_shape = seq_info[key0]["image_size"][::-1]
        aspect_ratio = float(image_shape[1]) / image_shape[0]
        image_shape = 1024, int(aspect_ratio * 1024)
        # relative path of visualization
        visual_path = "../visual/" + exp_name + "/" + seq_info[key0]["image_filenames"].split(":")[0] # Take the file name before : and discard the id number after :

        self.viewer = ImageViewer(
            update_ms, image_shape, visual_path)
    ...
    def draw_trackers(self, tracks):
        ...
        for track in tracks:
            ...
            self.viewer.rectangle_bbox_score(
                *track.to_tlwh().astype(np.int), track.score_attr, track.score_text, track.score_total, track.score_total_hits,label=str(track.track_id))
