...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def __init__(self, update_ms, window_shape=(640, 480), caption="Figure 1"):
        ...
        self._view1_video_writer = None
        self._view2_video_writer = None
        self._view3_video_writer = None
        self._view4_video_writer = None
    ...
    def rectangle_bbox_score(self, x, y, w, h, score_attr, score_text, score_total, score_total_hits, label=None):
        """Draw a rectangle and two scores including score_attr, score_text and score_total
        ...
        if label is not None:
            ...
            pt1 = int(x), int(y - 5 - text_size[0][1]) 

            center = pt1[0] + 5, pt1[1] + text_size[0][1] - 2

            pt1 = int(x - 1), int(y - 3*text_size[0][1] - 10) 

            pt2 = pt1[0] + 3*text_size[0][0] - 10, pt1[1] + 3*text_size[0][1] + 10 

            cv2.rectangle(self.image, pt1, pt2, self._color, -4)
            cv2.putText(self.image, label, center, cv2.FONT_HERSHEY_PLAIN,
                        2.5, (255, 255, 255), self.thickness) 
    # ######################################################################################################### #
    ...
    def enable_videowriter(self, output_filename, fourcc_string="mp4v",
        ...
        fps = 20 # Default 20 frames per second
        if 'View1' == view:
            self._view1_video_writer = cv2.VideoWriter(
                output_filename, fourcc, fps, (self._window_shape[1], self._window_shape[0]))
        if 'View2' == view:
            self._view2_video_writer = cv2.VideoWriter(
                output_filename, fourcc, fps, (self._window_shape[1], self._window_shape[0]))
        if 'View3' == view:
            self._view3_video_writer = cv2.VideoWriter(
                output_filename, fourcc, fps, (self._window_shape[1], self._window_shape[0]))
        if 'View4' == view:
            self._view4_video_writer = cv2.VideoWriter(
                output_filename, fourcc, fps, (self._window_shape[1], self._window_shape[0]))
    ...
    def run(self, update_fun=None):
        ...
        while not self._terminate:
            ...
            if not is_paused:
                ...
                # Save Video
                if len(self._caption.split("/")) > 3:
                    save_video_filename = '/'.join(self._caption.split("/")[0:-2]) + "/video/" + '/'.join(self._caption.split("/")[-2:])
                if len(self._caption.split("/")) == 3:
                    save_video_filename = self._caption.split("/")[0] + "/video/" + '/'.join(self._caption.split("/")[-2:])
                    
                if not os.path.exists(save_video_filename):
                    os.makedirs(save_video_filename)

                if "View1" == self.view:
                    output_filename = save_video_filename + "/" + str(self.view) + ".mp4"
                    if self._view1_video_writer is None:
                        self.enable_videowriter(output_filename, view=self.view)
                    else:
                        assert("Empty")
                    if self._view1_video_writer is not None:
                        self._view1_video_writer.write(
                            cv2.resize(self.image, (self._window_shape[1], self._window_shape[0])))
                if "View2" == self.view:
                    output_filename = save_video_filename + "/" + str(self.view) + ".mp4"
                    if self._view2_video_writer is None:
                        self.enable_videowriter(output_filename, view=self.view)
                    else:
                        assert("Empty")
                    if self._view2_video_writer is not None:
                        self._view2_video_writer.write(
                            cv2.resize(self.image, (self._window_shape[1], self._window_shape[0])))
                if "View3" == self.view:
                    output_filename = save_video_filename + "/" + str(self.view) + ".mp4"
                    if self._view3_video_writer is None:
                        self.enable_videowriter(output_filename, view=self.view)
                    else:
                        assert("Empty")
                    if self._view3_video_writer is not None:
                        self._view3_video_writer.write(
                            cv2.resize(self.image, (self._window_shape[1], self._window_shape[0])))
                if "View4" == self.view:
                    output_filename = save_video_filename + "/" + str(self.view) + ".mp4"
                    if self._view4_video_writer is None:
                        self.enable_videowriter(output_filename, view=self.view)
                    else:
                        assert("Empty")
                    if self._view4_video_writer is not None:
                        self._view4_video_writer.write(
                            cv2.resize(self.image, (self._window_shape[1], self._window_shape[0])))
            t1 = time.time()
            ...
            # If the image is not empty, write the image
            if self.image is not None:
                # Use .jpg image format to save images and save memory
                cv2.imwrite(os.path.join(save_name, self.frame_id + '.jpg'), cv2.resize(self.image, (self._window_shape[1], self._window_shape[0])))
