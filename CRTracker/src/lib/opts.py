...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument("--exp_id", default="CRTracker", help="CRTracker")
        self.parser.add_argument("--test", action="store_true")
        self.parser.add_argument(
            "--load_model",
            default="../models/CrossMOT_models/model_last.pth",
        ...
        self.parser.add_argument(
            "--CNN_image_alpha", type=float, default=0.01, help="CNN_image_features's coefficient"
        )
        self.parser.add_argument(
            "--score_text_gamma", type=float, default=1, help="score_text's coefficient"
        )
        self.parser.add_argument(
            "--score_attr_beta", type=float, default=0.1, help="exp(score_attr)'s coefficient"
        )
        self.parser.add_argument(
            "--text_prompt", type=str, default="text_prompt_4", help="prompt of text's input."
        )
        self.parser.add_argument(
            "--lr", type=float, default=1e-4, help="learning rate for batch size 12."
        )
        self.parser.add_argument(
            "--lr_step", type=str, default="20", help="drop learning rate by 10. use , as separator. each time the learning rate is reduced by 10 times."
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--test_campus", action="store_true", help="Track CAMPUS dataset"
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=0,
            help="Whether the inference result is visualized; 0 | 1; 0 means disabled, 1 means enabled.",
        )
        self.parser.add_argument(
            "--text_score_thres",
            type=float,
            default=0,
            help="text score thresh for tracking",
        )
        self.parser.add_argument(
            "--attr_score_thres",
            type=float,
            default=0,
            help="attr score thresh for tracking",
        )
        self.parser.add_argument(
            "--average_views_total_score_thres",
            type=float,
            default=0.5,
            help="threshold of average fusion score",
        )
        self.parser.add_argument(
            "--single_view_total_score_thres",
            type=float,
            default=0.75,
            help="threshold of single-view fusion score",
        )
        self.parser.add_argument(
            "--score_total_hits_thres",
            type=float,
            default=30,
            help="threshold of hit score",
        )
        self.parser.add_argument(
            "--hits_1",
            type=float,
            default=3,
            help="average hit score",
        )
        self.parser.add_argument(
            "--hits_2",
            type=float,
            default=3,
            help="single-view hit score",
        )
        self.parser.add_argument(
            "--hits_3",
            type=float,
            default=1,
            help="single-view miss score",
        )
        self.parser.add_argument(
            "--conf_thres",
            type=float,
        ...
        self.parser.add_argument(
            "--data_cfg",
            type=str,
            default="../src/lib/cfg/CRTrack_In-domain.json",
            help="load data from cfg",
        )
        self.parser.add_argument(
            "--data_dir", type=str, default="/mnt/A/hust_csj/Code/Github/CRMOT/datasets/CRTrack"
        )
        ...
        if opt.task == "mot":
            if opt.baseline == 0:
                opt.heads = {
                    ...
                    "text_id": 256 if opt.task_model == "APTM" else 512, # APTM                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
                }
            else:
